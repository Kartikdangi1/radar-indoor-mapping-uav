...
public:
    ...
    // Configurable parameters for sparse radar clouds
    void setICPParams(size_t min_icp_points, double max_fitness, double max_corr_dist) {
        min_icp_points_ = min_icp_points;
        max_icp_fitness_ = max_fitness;
        max_correspondence_dist_ = max_corr_dist;
    }
    ...
    void update_constraints(int window_size) {
        ...
        // Parameters optimized for SPARSE radar point clouds
        pcl::GeneralizedIterativeClosestPoint<PointT, PointT> reg;
        reg.setMaximumIterations(100);              // More iterations for convergence
        reg.setTransformationEpsilon(1e-8);         // Tighter convergence
        reg.setMaxCorrespondenceDistance(max_correspondence_dist_);  // Configurable
        reg.setCorrespondenceRandomness(4);         // Low for sparse clouds
        reg.setRANSACIterations(20);                // More RANSAC iterations
        ...
        #pragma omp parallel for num_threads(4) reduction(+:constraints_added)
        for (int i = 1; i <= window_size - 1; i++) {
            int id_target = (*keyframes_)[n - i].getId();

            // Downsample the point clouds - use smaller voxel for sparse data
            ...
            voxel_grid.setLeafSize(0.15f, 0.15f, 0.15f);  // Slightly larger voxels
            ...
            // Skip ICP for very small clouds
            if (downsampled_source_cloud->size() < min_icp_points_ || 
                downsampled_target_cloud->size() < min_icp_points_) {
                std::cerr << "Skipping ICP for nodes " << id_source << " and " << id_target
                          << " - insufficient points (" << downsampled_source_cloud->size()
                          << ", " << downsampled_target_cloud->size() << ")" << std::endl;
                continue;
            }
            ...
            // Initial guess for GICP based on poses (CRITICAL for convergence)
            ...
            // Check convergence
            if (!reg.hasConverged()) {
                std::cerr << "ICP did not converge for nodes: " << id_source 
                          << " and " << id_target << std::endl;
                continue;
            }

            if (aligned_cloud->empty()) {
                std::cerr << "ICP returned empty cloud for nodes: " << id_source 
                          << " and " << id_target << std::endl;
                continue;
            }
            ...
            if (!std::isfinite(fitness_score)) {
                std::cerr << "ICP returned non-finite fitness for nodes: " << id_source 
                          << " and " << id_target << std::endl;
                continue;
            }
            ...
            // Adaptive weighting based on fitness score
            // Lower fitness = better match = higher weight
            double weight = 1.0 / (fitness_score + 0.1);  // Added 0.1 to avoid division issues

            // If the fitness score is acceptable, add the ICP constraint
            if (fitness_score < max_icp_fitness_) {
                ...
                std::cout << "ICP constraint added: " << id_source << " -> " << id_target 
                          << " (fitness: " << fitness_score << ", weight: " << weight << ")" << std::endl;
            } else {
                // Log but don't completely reject - the odometry constraint will still be used
                std::cerr << "ICP fitness too high (" << fitness_score << ") for nodes: " 
                          << id_source << " and " << id_target 
                          << " - using odometry-only constraint" << std::endl;
            }
        }

        if (constraints_added == 0) {
            std::cerr << "WARNING: No ICP constraints added for keyframe " << id_source 
                      << ". Graph relies on odometry only." << std::endl;
        }
    }
    ...
    void create_graph(int window_size)
    {
        ...
        for (int i = 0; i < window_size; ++i)
        {
            ...
            if (firstQuat) {
                ...
                graph_.SetParameterization(&params_(window_position_node,0),
            ...
            // IMU constraint (roll/pitch from accelerometer)
            ...
            graph_.AddResidualBlock(cf_imu, nullptr,
                                  &params_(window_position_node,0));

            // Odometry constraint (sequential keyframes)
            if (window_position_node > 0)
            {
                ...
                if (quat_done.insert(window_position_related).second) {
                    ...
                    graph_.SetParameterization(&params_(window_position_related,0),
                                       new ceres::QuaternionParameterization());
                }
                ...
                // Odometry constraint with higher weight when ICP fails
                // This ensures the graph is always connected
                double odom_weight = 5.0;
                if (node.getConstraints().empty()) {
                    odom_weight = 10.0;  // Increase weight when no ICP constraints
                }
                
                ceres::CostFunction* cf_odom =
                    CostFunctionOdom::Create(node.get_Odom_tf().block<3,1>(0,3), odom_weight);
                graph_.AddResidualBlock(cf_odom, nullptr,
                    &params_(window_position_related,4),
                    &params_(window_position_node,   4),
                    &params_(window_position_related,0));
            ...
            // Loop closure constraints from ICP (GICP)
            for (size_t j = 0; j < constraints_number; ++j)
            {
                ...
                // Orientation constraint from ICP
                ceres::LossFunction* loss_function_ori = new ceres::HuberLoss(0.5);
                ...
                // Translation constraint from ICP
                ceres::LossFunction* loss_function_trans = new ceres::HuberLoss(0.5);
...
private:
    ...
    size_t min_icp_points_ = 8;
    double max_icp_fitness_ = 5.0;
    double max_correspondence_dist_ = 3.0;
